...
if (isset($_POST['ubah'])) {
    ...
    if (!empty($gambar_baru) && $error === 0) {
        ...
        if (!in_array($ekstensi, $ekstensi_valid)) {
            echo "<script>alert('❌ File harus berupa JPG, JPEG, atau PNG!'); window.location.href='ubah-program.php?kode=$id_promdat';</script>";
            exit;
        }

        if ($ukuran > 2 * 1024 * 1024) {
            echo "<script>alert('⚠️ Ukuran file terlalu besar! Maksimal 2MB'); window.location.href='ubah-program.php?kode=$id_promdat';</script>";
            exit;
        }
        ...
        if (move_uploaded_file($tmp, $pathBaru)) {
            ...
            if (mysqli_query($koneksi, $query)) {
                echo "<script>alert('✅ Data berhasil diperbarui dengan gambar baru!'); window.location.href='../kelola-konten/program.php';</script>";
                exit;
            } else {
                // Jika query gagal, hapus file yang sudah diupload
                unlink($pathBaru);
                echo "<script>alert('❌ Gagal update database: " . mysqli_error($koneksi) . "'); window.location.href='ubah-program.php?kode=$id_promdat';</script>";
                exit;
            }
        } else {
            echo "<script>alert('❌ Gagal memindahkan file ke folder tujuan!'); window.location.href='ubah-program.php?kode=$id_promdat';</script>";
            exit;
        }
    } else {
        // Jika tidak upload gambar baru atau ada error upload
        if ($error !== 0 && $error !== 4) { // Error 4 = No file uploaded
            echo "<script>alert('❌ Error upload file: $error'); window.location.href='ubah-program.php?kode=$id_promdat';</script>";
            exit;
        }
        ...
        if (mysqli_query($koneksi, $query)) {
            echo "<script>alert('✅ Data berhasil diperbarui tanpa ubah gambar!'); window.location.href='../kelola-konten/program.php';</script>";
            exit;
        } else {
            echo "<script>alert('❌ Gagal update database: " . mysqli_error($koneksi) . "'); window.location.href='ubah-program.php?kode=$id_promdat';</script>";
            exit;
                ...
                <form class="space-y-4 w-full lg:w-2/5 mb-32" action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <?php if (!empty($tampilProgram['img_card']) && file_exists("../../assets/img/promdat/" . $tampilProgram['img_card'])): ?>
                            <img src="../../assets/img/promdat/<?= $tampilProgram['img_card']; ?>" alt="Preview Gambar" width="200"
